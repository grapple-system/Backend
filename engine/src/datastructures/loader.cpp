		...
		if (fp != NULL) {
			...
			while (0 != fread(&src, 4, 1, fp)) {
				...
				for (int i = 0; i < degree; i++) {
					...
					currsiz = *((int*)(bbuf+5));
					free(bbuf);
					bbuf = (char *)malloc(currsiz);
					fread(bbuf, 1, currsiz, fp);
					constrRep.append(bbuf, currsiz);
					free(bbuf);

					outEdges.push_back(dst);
					outEdgeValues.push_back(label);
					PseudoPC ppc = PseudoPC(constrRep);
					if (ppc.empty()) {
						std::cerr << "empty PseudoPC found" << endl;
						exit(1);
					}
					constraints.push_back(ppc);
					constrRep.clear();
				}
			...
			// #endif
			// assert(false, "cant open bin file ?");
		}
	}
