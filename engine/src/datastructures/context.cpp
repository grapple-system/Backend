...
Context::Context(int argc, char** argv) {
	...
	for (int i = 0; i < argc; i++) {
		...
		else if (!strncmp(argv[i], SMT2_FILE_KEY.c_str(), sizeof(argv[i]))) {
			filePath.insert(std::make_pair(SMT2_FILE_KEY, context));
			cout << "SMT2_FILE_KEY = " << context << endl;
		}
...
string Context::getSEGFiles() {
	return filePath[SMT2_FILE_KEY];
}
