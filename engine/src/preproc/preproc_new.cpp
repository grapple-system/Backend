...
void Preproc_new::setVIT(Context &context)
{
	...
	mSize = (unsigned long long int)numVertex * (unsigned long long int)423 + (unsigned long long int)count*(unsigned long long int)BytesPerEdge;
	...
	for (i = 0; i < dataSize; i++) {
		...
		sum += ((unsigned long long int)(dataCount[i] + temp)*(unsigned long long int)BytesPerEdge + (unsigned long long int)423 );
	...
	for (i = 0; i < dataSize; i++) {
		if (dataCount[i] == 0)
			continue;
		sum += dataCount[i];
		mSum += (unsigned long long int)dataCount[i]* (unsigned long long int)BytesPerEdge + (unsigned long long int)423;
		...
		if (f != NULL) {
			while (0 != fread(&src, 4, 1, f)) {
				...
				for (int n = 0; n < degree; n++) {
					constrBBuf = (char*)malloc(constrSizes[n]);
					fread(constrBBuf, 1, constrSizes[n], f);
					constrStr.append(constrBBuf, constrSizes[n]);
			...
			// string str3 = "cant open bin file ";
			// str3 += str;
			// assert(false, str3.c_str());
		}
		numVert = 0;
		str2 = std::to_string((long long)i++);
		name = context.getGraphFile() + "." + PART + "." + BINA + "." + str.c_str() + "." + str2.c_str();
