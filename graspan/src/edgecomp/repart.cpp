...
void Repart::repartition(Partition &p1, Partition &p2, Context &context) {
	unsigned long long int size = (unsigned long long int)(p1.getNumVertices()) * (unsigned long long int)423 + (unsigned long long int)(p1.getNumEdges()) * (unsigned long long int)(BytesPerEdge);
	...
	check += dataTemp[0].getNumOutEdges() * BytesPerEdge;
	check += 423;
	for (i ; i < p1.getNumVertices() - 1; i++) {
		sum += dataTemp[i].getNumOutEdges();
		check += dataTemp[i + 1].getNumOutEdges() * BytesPerEdge;
		check += 423;
		if (check > size) {
			break;
		}
...
void Repart::run(Partition &p1, Partition &p2, Context &context, long newIterEdges) {
	...
	if (p1_2.getExist()) {
		...
		p1_2.calc_ddr_s(p1.getID(),context);
	...
	if (p2_2.getExist()) {
		...
		p2_2.calc_ddr_s(p2.getID(),context);
...
void Repart::clear() {
	if (p1_2.getExist()) p1_2.clear();
	if (p2_2.getExist()) p2_2.clear();
}
