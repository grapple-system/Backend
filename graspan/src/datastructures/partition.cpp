	...
	if (readable) {
		...
		for (int i = 0; i < part.data.size(); i++) {
			...
			for (int j = 0; j < part.data[i].getNumOutEdges(); j++) {
				...
				constraint = part.data[i].getConstr(j);
				fwrite((const void*) & dst, sizeof(int), 1, fp);
				fwrite((const void*) & label, sizeof(char), 1, fp);

				constrStr = constraint.getConstrRep();
				currsiz = constrStr.size();
				fwrite((const void*) &currsiz, sizeof(int), 1, fp);
				fwrite((const void*) constrStr.c_str(), 1, currsiz, fp);
				constrStr.clear();
			}
		}
		fclose(fp);
...
void Partition::calc_ddr(Context &context)	{
	...
	ddmMap[id].assign(ddmMap[id].size(), 0);
...
void Partition::calc_ddr_s(int opid, Context &context)	{
	if (!exist)
		return;
	vector<vector<double> > &ddmMap = context.ddm.getDdmMap();
	vector<vector<double> > &d_ddmMap = context.ddm.getD_ddmMap();
